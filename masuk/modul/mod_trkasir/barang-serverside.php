...
if ($_GET['action'] == "table_data") {
    ...
    $querycount = $db->prepare("SELECT count(id_barang) as jumlah FROM barang");
    $querycount->execute();
    ...
    if (empty($_POST['search']['value'])) {
        $query = $db->prepare("SELECT id_barang,
                                    kd_barang,
                                    nm_barang,
                                    stok_barang,
                                    sat_barang,
        ...
        $querycount = $db->prepare("SELECT count(id_barang) as jumlah 
            FROM barang WHERE kd_barang LIKE '%$search%' 
                        OR nm_barang LIKE '%$search%'
                        OR stok_barang LIKE '%$search%'
                        OR sat_barang LIKE '%$search%'
    ...
    if (!empty($query)) {
        ...
        while ($value = $query->fetch(PDO::FETCH_ASSOC)) {
            ...
            $nestedData['hrgjual_barang']   = '<table><tr>
                                            ...
                                            <tr>
                                                <td><b>(D)</b> </td><td>'.format_rupiah($value['hrgjual_barang1']).'</td>
                                            </tr>
                                            <tr>
                                                <td><b>(M)</b> </td><td>'.format_rupiah($value['hrgjual_barang2']).'</td>
                                            </tr></table>';
