...
if ($_GET['action'] == "table_data") {
    ...
    $querycount = $db->prepare("SELECT count(id_trbmasuk) as jumlah FROM trbmasuk WHERE id_resto = 'pusat' AND jenis = 'pbf' AND kd_orders != ''");
    $querycount->execute();
    ...
    if (empty($_POST['search']['value'])) {
        ...
        $query = $db->prepare("SELECT * 
            FROM trbmasuk
            WHERE trbmasuk.id_resto = 'pusat' AND trbmasuk.jenis = 'pbf' AND kd_orders != ''
    ...
    if (!empty($query)) {
        ...
        while ($value = $query->fetch(PDO::FETCH_ASSOC)) {
            ...
            $nestedData['aksi'] = "<a href='?module=trbmasukpbf&act=evaluasi_tampil&id=$value[id_trbmasuk]' title='EDIT' class='btn btn-warning btn-xs'>TAMPIL</a>";
            $data[] = $nestedData;
            $no++;
        }
    }
