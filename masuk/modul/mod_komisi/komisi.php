...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
	...
	switch (isset($_GET['act']) ? $_GET['act'] : '') {
		...
		default:
			...
			$stmt = $db->query("SELECT * FROM barang WHERE komisi != 0 ");
			$tampil_komisi = $stmt->fetchAll(PDO::FETCH_ASSOC);
		...
		case "tutupkomisi":
			$stmt = $db->query("SELECT * FROM admin WHERE akses_level = 'petugas' ORDER BY id_admin ASC");
					...
					<table id="example1" class="table table-bordered table-striped">
						...
						<tbody>
							...
							foreach ($staff as $r):
								...
								$stmt = $db->prepare("SELECT SUM(ttl_komisi) as total_komisi, MIN(tgl_komisi) as min_date, MAX(tgl_komisi) as max_date
						                FROM komisi_pegawai WHERE id_admin = ? AND status_komisi = 'on'");
								$stmt->execute([$r['id_admin']]);
								$kms = $stmt->fetch(PDO::FETCH_ASSOC);
								?>
								<tr>
									<td><?= $no++ ?></td>
...
//    break
			...
			<div class="box box-primary box-solid">
				...
				<div class="box-body table-responsive">
					<form method="POST" action="<?= $aksi ?>?module=komisi&act=input_komisiglobal" enctype="multipart/form-data"
						...
						<div class='col-sm-3'>
							...
							<select name='status' id='status' class='form-control'>
								<?php
								$stmt = $db->query("select * from komisiglobal where id_komisiglobal=1");
					...
					<div>
						<div>
							<?php
							$stmt = $db->query("select * from komisiglobal where status='ON' ");
							$global1 = $stmt->fetch(PDO::FETCH_ASSOC);
							$kogo = $global1['nilai'];
						...
						<table id="example1" class="table table-bordered table-striped">
							...
							<tbody>
								<?php
								$stmt = $db->query("select * from admin where akses_level='petugas' and blokir='N' ");
								$admin1 = $stmt->fetchAll(PDO::FETCH_ASSOC);
								...
								foreach ($admin1 as $r) {
									...
									$stmt = $db->prepare("SELECT SUM(ttl_trkasir) as total_komisi FROM trkasir
                                where tgl_trkasir between ? and ? and petugas=? ");
									$stmt->execute([$awalbulan, $tgl_awal, $petugas]);
		...
		case "bulan":
			...
			$admin1 = $admin1->fetchAll(PDO::FETCH_ASSOC);
			?>
			<table id="example1" class="table table-bordered table-striped">
				<thead>
					<tr>
				...
				<tbody>
					...
					foreach ($admin1 as $t) {
						$stmt = $db->prepare("SELECT SUM(ttl_trkasir) as total_komisi FROM trkasir
                                where month (tgl_trkasir)=? and year (tgl_trkasir) =? and petugas=? ");
						$stmt->execute([$bulan, '2025', $t['nama_lengkap']]);
						$komisipetugas1 = $stmt->fetch(PDO::FETCH_ASSOC);
						$kp = $komisipetugas1['total_komisi'];

						$kg1 = $db->prepare("select * from komisiglobal where month (tgl)=? ");
						$kg1->execute([$bulan]);
						$kg2 = $kg1->fetch(PDO::FETCH_ASSOC);
						$angka = $kg2['nilai'];
