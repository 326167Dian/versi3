...
switch($_GET['act']){
  ...
  default:
    ...
    //   $tampil_trbmasuk = $db->prepare("SELECT * FROM trbmasuk 
...
<?php
	...
	$cekkd = $db->prepare("SELECT * FROM kdbm WHERE id_admin=? 
	                        AND id_resto=? 
	                        AND stt_kdbm=?");
    ...
    <div class="modal-content">
      ...
      <div class="modal-body table-responsive">
		<table id="example1" class="table table-condensed table-bordered table-striped table-hover" >
						...
						<?php 
						    $no=1;
							$tampil_dproyek = $db->prepare("SELECT * FROM barang ORDER BY id_barang ASC");
