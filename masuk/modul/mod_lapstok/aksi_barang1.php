...
echo "<script type='text/javascript'>alert('Kode dengan Nama Barang dan Satuan ini sudah ada!');history.go(-1);</script>";
    ...
    $stmt_insert_barang = $db->prepare("INSERT INTO barang(kd_barang,
										 nm_barang,
										 stok_barang,
										 stok_buffer,
										 sat_barang,
										 ...
										 ket_barang)
								 VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	$stmt_insert_barang->execute([$_POST['kd_barang'], $_POST['nm_barang'], $_POST['stok_barang'], $_POST['stok_buffer'], $_POST['sat_barang'], $_POST['jenisobat'], $_POST['hrgsat_barang'], $_POST['hrgjual_barang'], $_POST['tgl_expired'], $_POST['ket_barang']]);
 ...
 elseif ($module=='barang' AND $act=='update_barang'){
     ...
     $stmt_update_barang = $db->prepare("UPDATE barang SET kd_barang = ?,
									nm_barang = ?,
									stok_barang = ?,
									stok_buffer = ?,
									sat_barang = ?,
									...
									ket_barang = ?
									WHERE id_barang = ?");
	$stmt_update_barang->execute([$_POST['kd_barang'], $_POST['nm_barang'], $_POST['stok_barang'], $_POST['stok_buffer'], $_POST['sat_barang'], $_POST['jenisobat'], $_POST['hrgsat_barang'], $_POST['hrgjual_barang'], $_POST['tgl_expired'], $_POST['ket_barang'], $_POST['id']]);
...
elseif ($module=='barang' AND $act=='hapus'){
  ...
  $stmt_delete_barang = $db->prepare("DELETE FROM barang WHERE id_barang = ?");
  $stmt_delete_barang->execute([$_GET['id']]);
