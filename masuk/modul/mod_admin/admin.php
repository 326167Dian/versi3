	...
	switch ($_GET['act']) {
		...
		default:
			if ($_SESSION['level'] == 'pemilik') {
				$tampil_admin = $db->prepare("SELECT * FROM admin ORDER BY username");
				$tampil_admin->execute();
?>
				<!--<div class="card card-primary box-solid">-->
					...
					<!--<div class="card-body table-responsive">-->
					    ...
					    <div class="row">
    					    <div class="col-auto">
    						    <a class='btn btn-success' href='?module=admin&act=tambahadmin'>Tambah</a>
                            </div>
                            <div class="col-auto">
    						    <a class='btn btn-info' href='?module=admin&act=loglogin'>Log Login</a>
                            </div>
                        </div>
...
<?php
		...
		case "loglogin":
			if ($_SESSION['level'] == 'pemilik') {
				$tampil_log = $db->query("SELECT * FROM user_login_logs ORDER BY login_time DESC");
?>
				<div class="box box-primary box-solid">
					<div class="box-header with-border">
						<h3 class="box-title">Log Login/Logout</h3>
					...
					<div class="box-body table-responsive">
						<table id="example1" class="table table-bordered table-striped">
							...
							<tbody>
								<?php
								$no = 1;
								while ($r = $tampil_log->fetch(PDO::FETCH_ASSOC)) {
									$logout_time = $r['logout_time'] ? $r['logout_time'] : 'Still Active';
									$duration = '';
									if ($r['logout_time']) {
										$login_ts = strtotime($r['login_time']);
										$logout_ts = strtotime($r['logout_time']);
										$diff = $logout_ts - $login_ts;
...
<?php
		...
		case "editadmin":
			$edit = $db->query("SELECT * FROM admin WHERE id_admin='$_GET[id]'");
